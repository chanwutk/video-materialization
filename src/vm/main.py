...
def run_experiment(args):
    ...
    for policy in policies:
        print(f"\nPolicy: {policy.value}")
        for vid in tqdm(selected, desc=f"  {policy.value}"):
            youtube_url = f"https://www.youtube.com/watch?v={vid}"
            segs = video_segments[vid]
            ...
            for entry in selected[vid]:
                predicted_id, raw_response, usage = answer_question(
                    client, policy, youtube_url, vid, segs, materialized, entry, model=model,
                )
